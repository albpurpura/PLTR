...
class ReRanker:
    def __init__(self, seed, learning_rate, n_heads, num_features, loss_fn, list_size, coll_name, det_model=False,
        ...
        with tf.variable_scope('encoder1'):
            ...
            self.multi_head_satt0 = MultiHeadSelfAttLayer(self.n_heads, self.input_docs.shape[-1], self.hidden_size, 0)
        with tf.variable_scope('encoder0'):
            self.multi_head_satt1 = MultiHeadSelfAttLayer(self.n_heads, self.input_docs.shape[-1], self.hidden_size, 0)
        ...
        _, self_att_0_out = self.multi_head_satt0(self.input_docs, self.training)
        _, self_att_1_out = self.multi_head_satt1(self_att_0_out, self.training)
        # joint = tf.multiply(left_side, right_side)

        self.logits = tf.squeeze(self.output_layer(self_att_1_out), axis=-1)
